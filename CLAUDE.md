...
| Path | What |
|---|---|
| `bin/swarm` | Launcher/CLI (Babashka): init/switch/toolset/up/bootstrap/prompt/squad/logs/down/status |
...
| `toolsets/*.edn` | Quality-tool profiles keyed by purpose (dotnet, clojure) |
| `test/smoke.sh` | 200+ checks; the executable spec. MUST be green before any commit |
| `docs/` | Designs: squad-v2/s3/s4, choosing-a-mode | 
...
daemons + squad events + handoff lifecycles); `swarm down` stops
everything. Squad approvals: `swarm squad approvals|approve|deny|report`.
